...
void job_dispatch(int i){
    ...
    // TODO#3:  a. Always check the corresponding shmPTR_jobs_buffer[i] for new
    //             jobs from the main process
    //          b. Use semaphore so that you don't busy wait
    //          c. If there's new job, execute the job accordingly: either by
    //             calling task(), usleep, exit(3) or kill(getpid(), SIGKILL)
...
void setup(){
    ...
    // 1a. Create shared memory for global_data struct (see processManagement_lab.h)
    ...
    // 1b. When shared memory is successfully created, set the initial values of "max" and "min" of the global_data struct in the shared memory accordingly
    //set global data min and max
    ShmPTR_global_data->max = -1;
    ShmPTR_global_data->min = INT_MAX;

    // 1c. Create semaphore of value 1 which purpose is to protect this global_data struct in shared memory
    // create a semaphore of name "semglobaldata" of init size 1. Semaphores must be unique by name
    sem_global_data = sem_open("semglobaldata", O_CREAT | O_EXCL, 0644, 1);
    // check if failed,
    while (sem_global_data == SEM_FAILED){
      // try to unlink, chaces are it failed because there's already a semaphore of such name (from your previously executed program, etc)
      sem_unlink("semglobaldata");
      // try to open again
      sem_global_data = sem_open("semglobaldata", O_CREAT | O_EXCL, 0644, 1);
    };

    // 1d. Create shared memory for number_of_processes job struct (see processManagement_lab.h)
    ...
    // 1e. When shared memory is successfully created, setup the content of the structs (see handout)
    // create a semaphore of name "semglobaldata" of init size 1. Semaphores must be unique by name
...
void main_loop(char* fileName){
    ...
    while (fscanf(opened_file, "%c %ld\n", &action, &num) == 2) { //while the file still has input
        ...
        while (not_done) {
          for (int i = 0; i<number_of_processes; i++){
            alive = waitpid(children_processes[i], &status, WNOHANG);
    ...
    while (terminated_children < number_of_processes) {
      for (int i = 0; i<number_of_processes; i++) {
        ...
        if (shmPTR_jobs_buffer[i].task_status == 0 && alive == 0) {
          ...
          shmPTR_jobs_buffer[i].task_status = 0; //new, undone job
          sem_post(sem_jobs_buffer[i]);
          terminated_children++;
        } else if (alive == 9) {
          terminated_children++;
...
void cleanup(){
    ...
    shmdt(ShmPTR_global_data);
    shmdt(shmPTR_jobs_buffer);
